...
static std::vector<T> loadVector(uint8_t* in, int offset, dsl_Vector& vec)
{
	...
	auto        size = static_cast<size_t>(vec.ElementCount) * sizeof(T);
...
namespace blt::db
{
	std::vector<DieselBundle*> DieselDB::bundles;
}
...
std::vector<uint8_t> DslFile::ReadContents(std::istream& fi) const
{
	...
	if (bundle->ChunkOffsets.empty())
	{
		...
		if (data.size() < sizeof(uint32_t))
			throw std::runtime_error("Compressed data too small to contain size header");

		uint32_t dstSize = *reinterpret_cast<uint32_t*>(data.data() + (data.size() - sizeof(uint32_t)));

		if (dstSize > 256 * 1024 * 1024)
			throw std::runtime_error("Decompressed size too large, possible corrupted data");
	...
	else
	{
		...
		while (destFileOffset < realLength)
		{
			...
			if (compressedData.size() < sizeof(uint32_t))
				throw std::runtime_error("Compressed block too small to contain size header");

            uint32_t dstSize = *reinterpret_cast<uint32_t*>(compressedData.data() + (compressedData.size() - sizeof(uint32_t)));

			if (dstSize > 256 * 1024 * 1024)
				throw std::runtime_error("Decompressed block size too large, possible corrupted data");
			...
			auto actualDestSize = static_cast<size_t>(destSize);
			auto dataPtr = data.data();

			if (destFileOffset == 0)
			{
				if (actualDestSize <= bufferOffset)
					throw std::runtime_error("Decompressed data smaller than buffer offset");
...
static void loadPackageHeader(DieselBundle* bundle, const std::string& headerPath, const std::string& dataPath, FileList files)
{
	...
	if (compressedSize < sizeof(uint32_t))
		throw std::runtime_error("Compressed package header too small");

	std::vector<uint8_t> srcData(compressedSize);
	in.read((char*)srcData.data(), srcData.size());

	if (srcData.size() < sizeof(uint32_t))
		throw std::runtime_error("Compressed package header data too small to contain size header");

	uint32_t dstSize = *reinterpret_cast<uint32_t*>(srcData.data() + (srcData.size() - sizeof(uint32_t)));

	if (dstSize > 64 * 1024 * 1024)
		throw std::runtime_error("Package header decompressed size too large, possible corrupted data");
...
static void loadBundleHeader(DieselBundle* dieselBundle, const std::string& headerPath, const std::string& dataPath, FileList files)
{
	...
	if (compressedSize < sizeof(uint32_t))
		throw std::runtime_error("Compressed bundle header too small");

	std::vector<uint8_t> srcData(compressedSize);
	in.read((char*)srcData.data(), srcData.size());

	if (srcData.size() < sizeof(uint32_t))
		throw std::runtime_error("Compressed bundle header data too small to contain size header");

	uint32_t dstSize = *reinterpret_cast<uint32_t*>(srcData.data() + (srcData.size() - sizeof(uint32_t)));

	if (dstSize > 64 * 1024 * 1024)
		throw std::runtime_error("Bundle header decompressed size too large, possible corrupted data");
...
static DieselBundle* loadPackageBundle(const std::string& dataPath)
{
	...
	DieselDB::bundles.push_back(bundle);
	return bundle;
}
